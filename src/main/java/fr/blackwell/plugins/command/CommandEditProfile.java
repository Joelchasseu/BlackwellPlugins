            ...
            if (index.has(usernameTarget)) {
                ...
                if (profile.has(tagToChange)) {
                    if (newValue.equals("true") || newValue.equals("false")) {
                        ...
                        profile.addProperty(tagToChange, newBool);
                    } else if (BWUtils.isNumeric(newValue)) {
                        int newInt = Integer.parseInt(newValue);
                        profile.addProperty(tagToChange, newInt);
                    } else {
                    ...
                    sender.sendMessage(new TextComponentString("Le tag " + tagToChange + " est introuvable pour le joueur " + usernameTarget).setStyle(new Style().setColor(TextFormatting.RED)));
            } else
                sender.sendMessage(new TextComponentString("Le joueur " + usernameTarget + " est introuvable dans le fichier players.json").setStyle(new Style().setColor(TextFormatting.RED)));
        } else
            sender.sendMessage(new TextComponentString("Execution impossible, veuillez respecter la structure suivante : /edit <username> <tagToChange> <newValue>").setStyle(new Style().setColor(TextFormatting.RED)));
    }
